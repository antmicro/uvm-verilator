...
`define UVM_SEQ_ITEM_PULL_IMP(imp, REQ, RSP, req_arg, rsp_arg) \
  function void disable_auto_item_recording(); endfunction \
  function bit is_auto_item_recording_enabled(); return 1'b1; endfunction \
  task get_next_item(output REQ req_arg);  endtask \
  task try_next_item(output REQ req_arg);  endtask \
  function void item_done(input RSP rsp_arg = null);  endfunction \
  task wait_for_sequences();  endtask \
  function bit has_do_available(); return 1'b1; endfunction \
  function void put_response(input RSP rsp_arg);  endfunction \
  task get(output REQ req_arg);  endtask \
  task peek(output REQ req_arg);  endtask \
  task put(input RSP rsp_arg);  endtask
